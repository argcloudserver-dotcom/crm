...
/**
 * Register a getter that supplies a CSRF token. Before every fetch,
 * the getter is invoked; when it returns a non-null string, an
 * `x-csrf-token` header is attached to the request.
 *
...
/**
 * Public accessors so other helpers (e.g. `apiFetch`) can attach the same
 * CSRF / bearer tokens that `customFetch` attaches, without duplicating
 * the getter wiring.
 */
export async function getCsrfTokenForRequest(): Promise<string | null> {
  if (!_csrfTokenGetter) return null;
  return (await _csrfTokenGetter()) ?? null;
}
...
export async function customFetch<T = unknown>(
  ...
  // Attach CSRF token when available
  ...
  // For web clients, also try to extract CSRF token from cookie as fallback
  ...
  // Unwrap API response envelope if present
